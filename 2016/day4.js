...
function lettersFrom (roomString) {
  return roomString.replace(/\[.*/,'').replace(/[^a-z]/g,'').split('')
}
function checksumFrom (roomString) {
  return roomString.match(/\[(.*)\]/)[1].split('')
}
function makeUnique (array) {
  var addIfMissing = (uniq, element) => {
    return uniq.includes(element) ? uniq : uniq.concat(element)
  }
  return array.reduce(addIfMissing, [])
}
function orderedFrequenciesFrom (histogram, entries) {
  return entries.map(key => histogram[key]).sort((a, b) => b - a)
}
function histogram (uniqueString, string) {
  let histo = {}
  uniqueString.forEach(letter => {
    histo[letter] = string.filter(l => l === letter).length
  })
  return histo
}
function sectorId (roomString) {
  return parseInt(roomString.match(/\d+/)[0])
}
function isValid (room) {
  let letters = lettersFrom(room)
  let checksum = checksumFrom(room)
  // then ensure the max of those is 1st in checksum
  let uniq = makeUnique(letters)
  let histo = histogram(uniq, letters)
  let frequencies = orderedFrequenciesFrom(histo, uniq)

  return checksum.every(function(current, index, array){
    if ([0, array.length - 1].includes(index)) {
      return histo[current] === frequencies[index]
    } else {
      return histo[current] === frequencies[index] &&
        (frequencies[index] > frequencies[index + 1] || current < array[index + 1])
    }
  })
}
function decode (char, n) {
  if (char.match(/[a-z]/)) {
    let val = char.charCodeAt(0) + n
    return val > 122 ? String.fromCharCode(val - 26) : String.fromCharCode(val)
  } else {
    return ' '
  }
}
function nameAndId (room) {
  let id = sectorId(room)
  let advancement = id % 26
  let name = room.split('').map(char => decode(char, advancement)).join('')
  return [name, id]
}
var total = rooms.filter(isValid).map(sectorId).reduce((tot, curr) => tot + curr)
console.log(total)
let names = rooms.filter(isValid).map(nameAndId).filter(results => results[0].includes('north'))
console.log(names)
